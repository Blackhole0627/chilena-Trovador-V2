...
(function ($) {
			...
			if (data) {
				...
				if (data.videoCallsIncoming) {
					if (data.videoCallsIncoming.status == 'calling') {
						...
						var incomingVideoModal = new bootstrap.Modal(document.getElementById('videoCallModalIncoming'), {
							backdrop: 'static',
							keyboard: false
						});
						...
						$('.modal-content-call').on('click', '#rejectCall', function (e) {
							...
							$.ajax({
								...
								if (data.status) {
									bootstrap.Modal.getInstance(document.getElementById('videoCallModalIncoming')).hide();
								} else {
									bootstrap.Modal.getInstance(document.getElementById('videoCallModalIncoming')).hide();
				...
				if (data.audioCallsIncoming) {
					if (data.audioCallsIncoming.status == 'calling') {
						...
						var incomingAudioModal = new bootstrap.Modal(document.getElementById('audioCallModalIncoming'), {
							backdrop: 'static',
							keyboard: false
						});
						...
						$('.modal-content-call').on('click', '#rejectAudioCall', function (e) {
							...
							$.ajax({
								...
								if (data.status) {
									bootstrap.Modal.getInstance(document.getElementById('audioCallModalIncoming')).hide();
								} else {
									bootstrap.Modal.getInstance(document.getElementById('audioCallModalIncoming')).hide();
									$('.popout').addClass('popout-error').html(error_occurred).slideDown('500');
								}
	...
	$(document).on('click', '.triggerSticker', function (e) {
		...
		var element = $(this);
		var container = element.siblings('.dropdown-stickers');
	...
	$(document).on('click', '.triggerGif', function (e) {
		...
		var element = $(this);
		var container = element.siblings('.dropdown-gifs');
